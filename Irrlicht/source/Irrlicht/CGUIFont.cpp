...
CGUIFont::~CGUIFont()
{
	...
	{
		//get Textures
		irr::core::array<irr::video::ITexture*> tex(SpriteBank->getTextureCount());
		for(irr::u32 i = 0; i<SpriteBank->getTextureCount(); i++){
			tex.push_back(SpriteBank->getTexture(i));//already allocated, no problem
		}
		//Clear Spritebank and remove old textures
		SpriteBank->clear();
		//Texturen löschen
		for(irr::u32 i = 0; i<tex.size(); i++){
			Driver->removeTexture(tex[i]);
		}
	}
