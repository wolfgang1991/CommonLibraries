	...
	: CIrrDeviceStub(param), Accelerometer(0), Gyroscope(0), Focused(false), Initialized(false), InitAborted(false), Paused(true), Stopped(false), IsSplitScreen(false), IsWindowInitialized(false), JNIEnvAttachedToVM(0), isInMultiWindowModeID(0)//changed
{
	...
	ButtonStates=0;
	if(Initialized){
		CursorControl = new CCursorControl(this, VideoDriver->getScreenSize());//must be set before smgr creation
	}
	//changed end
...
{
	//changed
	if(app==NULL || androidEvent==NULL){return 1;}//sth. is wrong, no dispatching
	CIrrDeviceAndroid* device = (CIrrDeviceAndroid*)app->userData;
	if(device==NULL){return 1;}
	if(device->CursorControl==NULL){return 1;}//not init, no dispatching
	//changed end
	...
	switch ( AInputEvent_getType(androidEvent) )
	{
		...
		case AINPUT_EVENT_TYPE_KEY:
		{
			...
			*/
			...
			// some keys (e.g. the " key) don't work if only known keys are used: if ( event.KeyInput.Key > 0 ) //changed
			{
				// TODO:
...
//changed end
			...
			device->postEventFromUser(event);
			if(keyCode == AKEYCODE_BACK){status = 1;}//changed (sonst Absturz bei <- Button)
		}
		break;
		default:
		break;
