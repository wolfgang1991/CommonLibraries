		...
		case EET_MOUSE_INPUT_EVENT:
			{
				...
				case EMIE_LMOUSE_LEFT_UP:
				{
					...
					//changed:
					if (state!=2 && isPointInside(p)){
						bool oldscroll = AutoScroll;
						AutoScroll = false;
						selectNew(event.MouseInput.Y);
						AutoScroll = oldscroll;
					}
					state = 0;
					//changed end
					return true;
				...
				case EMIE_MOUSE_MOVED:
					//changed:
					if(state>0){
						s32 newpos = oldScrollPos + py-event.MouseInput.Y;
						s32 minpos = ScrollBar->getMin();
						s32 maxpos = ScrollBar->getMax();
						if(abs(newpos-oldScrollPos)>=ItemHeight){state = 2;}
						ScrollBar->setPos(newpos<minpos?minpos:(newpos>maxpos?maxpos:newpos));
					}
					if ((Selecting || MoveOverSelect) && !event.MouseInput.isLeftPressed())
					{
						if (isPointInside(p))
						{
							bool oldscroll = AutoScroll;
